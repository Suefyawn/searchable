...
- Production deploys from GitHub: Workers Builds (Cloudflare dashboard, Worker `searchable-production`, Settings, Builds) watches `main`, runs `npm run build` with `NEXT_PUBLIC_SITE_URL=https://searchable.pk`, then `npx vinext-cloudflare deploy --env production --name searchable-production --config dist/server/wrangler.json --skip-build`. A push to `main` is a production deploy; `npm run deploy:production` from a machine is the manual fallback. It publishes the `production` environment. Until cutover its route stays unattached; Phase 9 of the migration plan attaches `searchable.pk`.
