...
7. Data: Dawn and Midday run POST /data {"ingest": true}. Any slot: when a notification gives a new number (OGRA fuel prices, SBP policy rate, NEPRA tariff, gold from a sarafa association), POST /data with readings and a sourceUrl. The Dawn ingest also reads SBP (policy rate, 12-month KIBOR, USD/PKR); on the Afternoon run compare the latest kibor-1y and sbp-policy-rate readings in /context with https://www.sbp.org.pk/ecodata/kibor_index.asp and POST a reading with the sourceUrl only if the site shows a different number. Never record a number you have not seen at its source.
