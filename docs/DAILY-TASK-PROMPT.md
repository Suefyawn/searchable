...
8. Directory (Midday only, now that the directory holds more than 300 listings): pick one city and one business category from /reference where /context.directory shows few or no listings (the big categories in Lahore, Karachi, Islamabad, Rawalpindi and Faisalabad first: hospitals, banks, car dealers, real estate agents, restaurants, schools), find 8 to 12 real businesses with a verifiable phone number and address (official website, Google Business listing, a directory you can cite), and POST /businesses with publish: true. Skip anything you cannot verify; never invent a phone number. Report what you added.
8b. Living comparisons (first fill: the next Midday run after 17 September 2026; then Midday on the 1st and 16th of the month, and the Afternoon after any SBP policy decision): GET /compare?slug=air-conditioners, ?slug=credit-cards, ?slug=mobile-packages and ?slug=national-savings. Refresh every price from the brand's own store or price list (Haier, Gree, Dawlance, Orient, Kenwood, PEL, TCL) and every fee, mark-up and minimum income from the bank's published schedule of charges and card page (HBL, UBL, MCB, Meezan, Bank Alfalah, Standard Chartered, Faysal, Askari, JS, Bank Islami). Mobile packages: the national prepaid bundles from Jazz, Zong, Telenor and Ufone package pages (price, data in GB, on-net and off-net minutes, SMS, validity, subscription code). National Savings: every scheme on the CDNS profit-rate sheet at savings.gov.pk (rate, payout, term, minimum, maximum, who, withholding), refreshed the day a new sheet appears. POST /compare with the full set, reviewedAt today and the source. Add models the brands list, drop ones they no longer sell. A number you cannot read on the brand's or bank's own page does not go in.
8c. Price lists (Afternoon, every day until mobiles hold 150 models, bikes 40 and cars 60; then Afternoon on Mondays and whenever a maker announces new prices): GET /prices?category=mobiles (or bikes, cars) to see what is there, then POST /prices {"category", "items": [...]} with models read from the makers' official Pakistan pages (Samsung, Vivo, Oppo, Infinix, Tecno, Xiaomi, Realme, Itel, Apple via an authorised reseller such as iStore or Mercantile; Atlas Honda, Pak Suzuki, Yamaha, United, Road Prince, Jolta; Pak Suzuki, Toyota Indus, Honda Atlas, Kia Lucky, Hyundai Nishat, Changan, MG, BYD, Haval). Each item: slug (brand-model, lowercase), brand, model, price (base variant, rupees), variants, specs (mobiles need ram, storage, battery; bikes and cars need engine; add display, chipset, camera, charging, mileage, transmission), url (the model page), source {title, url, publisher}, released (YYYY-MM). Upsert only what you verified today; a changed price is recorded to history by the server. Start with the models people search most (Semrush: the "price in pakistan" queries with the highest volume), 20 to 40 models per run. Never a price from a rumour site or a shop's WhatsApp; only official lists and authorised resellers. When a maker's page is JavaScript-only or refuses your fetcher, read it through POST /fetch (below): it returns the rendered page as markdown.
8d. Cricket today (Dawn and Evening, and any run while a Pakistan match is on): GET /match, then POST /match {"matches": [...]} with every fixture for today and the next seven days that Pakistani readers follow (Pakistan men and women, PSL, Asia Cup, ICC events, and the big India, England, Australia fixtures), each with id (e.g. pak-v-ind-2026-09-21), title ("Pakistan vs India"), competition, format, venue, startAt (ISO with +05:00), status (upcoming | live | finished | abandoned), watch (channels and apps in Pakistan), href (our living page for the series if there is one), sourceUrl (PCB, ICC or the league schedule). During a Pakistan match set status "live" with the score in "score" on every run; after it, "finished" with "result". Keep yesterday's results in the list; drop anything older. Send the whole list each time.
8e. Jobs (Morning and Night, 5 to 10 vacancies a run until the board holds 60 open ones, then whatever is new): GET /posts?kind=job to see what is listed, then POST /posts {"kind": "job", "title": "<role>, <employer>, <city>", "body": markdown (60 to 400 words: the post, who qualifies, pay and allowances, how and where to apply, the closing date), "company", "city" (slug), "topic" (e.g. government, banking, teaching, IT, defence), "employmentType", "salaryMin", "salaryMax", "applyUrl" (the official advertisement or careers page), "deadline": "YYYY-MM-DD"} for vacancies from official sources only: FPSC, PPSC, SPSC, KPPSC, BPSC, NTS, PTS, Pakistan Army, Navy and Air Force recruitment pages, PIA, NHA, WAPDA, banks' and large companies' careers pages, universities. Never from job aggregators, WhatsApp forwards or newspaper scans you cannot trace. Prefer roles with many posts and a clear closing date. When a closing date passes, POST /posts {"id", "intent": "close"}.
...
- The photo must show the subject, and a news story gets no photo rather than a loosely related one. Always send "image": {"query", "entities"} on a news story: "entities" are the exact proper names the story is about, most specific first (the person, the body, the building, the place: ["Kohat Police Lines", "Kohat"], ["Supreme Court of Pakistan"], ["Babar Azam", "Gaddafi Stadium"]); their Wikipedia photo is tried before any search. Make the "query" the concrete, photographable scene of this story ("Supreme Court of Pakistan building", "PSO petrol pump Lahore"), never a topic ("relief", "policy", "Pakistan"). The server ranks every candidate against the entities, the query and the headline and, for news, refuses anything that does not describe the story (flags, banknotes, skylines, maps and unrelated places included); the response then carries a "note" and the story is published without a photo. When that happens, look for the subject's own image (order of preference above) and PATCH /articles/{id} with {"image": {"url", "credit", "sourceUrl", "license"}}; do not retry with a vaguer query, and never settle for a generic scene on a news story. Guides and explainers may still use a scene for the topic ("cricket stadium Pakistan"). Download the returned image and look at it; if it does not show the subject, swap it, or PATCH /articles/{id} {"image": null} to remove it when nothing better exists.
...
PATCH /articles/{id} {"intent": "publish" | "unpublish" | "schedule", "scheduledFor"?}   or   {"image": {"query", "entities"} | {"url", "credit", "sourceUrl", "license"} | null} to swap only the photo (null removes a wrong or inappropriate photo at once; the story stands without one)   or   {"slug": "new-address"} to move a story whose address is now wrong (the old address redirects)
DELETE /articles/{id}
...
GET /prices?category=mobiles|bikes|cars[&brand=] → { reviewedAt, count, items[{slug,brand,model,price,variants,specs,url,source,released,history[{date,price}],updatedAt}] }

POST /prices {"category", "items": [...], "mode"?: "upsert"|"replace", "remove"?: ["slug"], "reviewedAt"?}   → { live, added, updated, removed, priceMoves }

GET /posts?kind=job&limit= → { posts[{id,slug,title,topic,url,company,deadline,publishedAt,expiresAt}] }   POST /posts {"kind":"job", title, body, company, city?, topic?, employmentType?, salaryMin?, salaryMax?, applyUrl, deadline?} → { id, url }   POST /posts {"id", "intent": "close"}

GET /match → { updatedAt, matches[...], today[], live[], upcoming[], recent[] }   POST /match {"matches": [{id,title,competition,format?,venue?,startAt,status,score?,result?,watch?,href?,sourceUrl}]}
...
POST /fetch {"url": "https://...", "format"?: "markdown" | "html" | "text", "waitFor"?: "<css selector>", "selector"?: "#id or .class", "timeoutMs"?: 20000} → { url, format, content, length, browserMs }   (a real browser on Cloudflare loads the page and runs its JavaScript; use it when a maker, bank or government page renders prices client-side or refuses your own fetcher: Samsung, Infinix, Tecno, Xiaomi, Yamaha, Suzuki, United, Chery, the DISCO bill sites. Browser time is metered, so plain fetch first, /fetch when that fails or comes back empty.)
