...
GET /compare?slug=air-conditioners|credit-cards|mobile-packages → { items[], reviewedAt, source }   POST /compare {"slug", "items": [{"id","brand","model","price","priceNote"?,"url","specs": {...},"note"?}], "reviewedAt", "source": {"title","url","publisher"}} (replaces the set; specs per slug in the notes above)
