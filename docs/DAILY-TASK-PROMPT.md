...
8b. Living comparisons (first fill: the next Midday run after 17 September 2026; then Midday on the 1st and 16th of the month, and the Afternoon after any SBP policy decision): GET /today → { date, pakistan{day,month,year,monthName}, umalqura{...}, offset{days,note,sourceUrl,setAt} }   POST /today {"days": -1|0|1, "note"?, "sourceUrl"?}
...
- Generated illustrations, guides only: when no openly licensed photo shows the subject of a guide, data page or explainer, generate one (Hugging Face or Higgsfield) of the thing itself (a meter, a bill on a table, a solar roof, a passport), never of a real person, place or event, and import it with {"url": ..., "credit": "Illustration: Searchable", "license": "generated"} and an alt that starts with "Illustration:". Never a generated image on a news story.
