...
import { escape, inviteUrl, layout } from "./claims";
...
export async function notifyBusinessLead(businessId: string, lead: { name: string; phone: string; message: string }) {
  ...
  if (!to) return;
  const body = `<p><strong>${escape(lead.name)}</strong> (${escape(lead.phone)}) sent this through your listing on ${SITE.name}:</p><blockquote style="margin:12px 0;padding:10px 14px;border-left:2px solid #111;color:#333">${escape(lead.message)}</blockquote>`;
  const tail = claimed
  ...
  await sendEmail({ to, subject: `New enquiry for ${b.name}: ${lead.name}`, html: layout(`Enquiry for ${escape(b.name)}`, body + tail), text: `${lead.name} (${lead.phone}): ${lead.message}` }).catch(() => {});
}
...
export async function notifyProfessionalLead(professionalId: string, lead: { name: string; phone: string; email?: string | null; message?: string | null }) {
  ...
  await sendEmail({
    ...
    html: layout(`Someone wants to reach you`, `<p><strong>${escape(lead.name)}</strong> (${escape(lead.phone)}${lead.email ? `, ${escape(lead.email)}` : ""}) sent this from your profile on ${SITE.name}:</p>${lead.message ? `<blockquote style="margin:12px 0;padding:10px 14px;border-left:2px solid #111;color:#333">${escape(lead.message)}</blockquote>` : ""}<p>Reply quickly; most people contact two or three professionals at once. <a href="${SITE.url}/professional">Your dashboard</a> keeps every enquiry.</p>`),
...
export async function notifyOutbid(postId: string, newAmount: number, newBidderId: string) {
  ...
  if (!user || !post) return;
  await sendEmail({ to: user.email, subject: `Outbid on ${post.title}`, html: layout(`You have been outbid`, `<p>Someone bid <strong>${pkr(newAmount)}</strong> on <a href="${SITE.url}/community/post/${post.slug}">${escape(post.title)}</a>. Bid again if you still want it.</p>`), text: `Outbid on ${post.title}: ${SITE.url}/community/post/${post.slug}` }).catch(() => {});
}
    ...
    for (const u of users) {
      ...
      try {
        await sendEmail({
          ...
          html: layout(`While you were away`, `<ul style="padding-left:18px;line-height:1.7">${items.map((i) => `<li><a href="${SITE.url}${i.url}">${escape(i.text)}</a></li>`).join("")}</ul><p style="color:#777;font-size:13px">One email a day at most. Manage what you get from <a href="${SITE.url}/account">your account</a>.</p>`),
