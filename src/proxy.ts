 ...
 * alternate, at the same URL. Browsers never send that Accept value, so readers are untouched. The matcher
 * keeps this off every other path so it costs nothing elsewhere.
 */
export function proxy(req: NextRequest) {
  const accept = req.headers.get("accept") ?? "";
  if (!/text\/markdown/i.test(accept) || /text\/html/i.test(accept.split(",")[0] ?? "")) return NextResponse.next();
  const url = req.nextUrl.clone();
  url.pathname = `/api/md${url.pathname}`;
