...
export async function categoryCounts(cityId?: string) {
  const db = await getDb();
  const rows = await db
    .select({
    ...
    .leftJoin(
      schema.businesses,
      and(eq(schema.businesses.primaryCategoryId, schema.businessCategories.id), eq(schema.businesses.status, "active"), cityId ? eq(schema.businesses.cityId, cityId) : undefined),
    )
...
export async function listBusinesses(opts: { categoryId?: string; cityId?: string; limit?: number; offset?: number; featuredFirst?: boolean } = {}): Promise<BusinessCard[]> {
  const db = await getDb();
...
export async function countBusinesses(opts: { categoryId?: string; cityId?: string } = {}) {
  const db = await getDb();
  const conds = [eq(schema.businesses.status, "active")];
  if (opts.categoryId) conds.push(eq(schema.businesses.primaryCategoryId, opts.categoryId));
  if (opts.cityId) conds.push(eq(schema.businesses.cityId, opts.cityId));
