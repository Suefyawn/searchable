...
export function ProLeadForm({ professionalId, name }: { professionalId: string; name: string }) {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const res = await sendProfessionalLead({ professionalId, name: String(fd.get("name") ?? ""), phone: String(fd.get("phone") ?? ""), email: String(fd.get("email") ?? ""), message: String(fd.get("message") ?? ""), website: String(fd.get("website") ?? "") });
