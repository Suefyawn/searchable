...
const ArticleInput = z.object({
  ...
  featuredImageSourceUrl: z.string().trim().max(500).optional(),
  ...
  contributorBio: z.string().trim().max(300).optional(),
...
export async function saveArticle(raw: ArticleFormInput): Promise<{ ok: boolean; error?: string; id?: string }> {
  ...
  const values = {
    ...
    featuredImageCredit: d.featuredImageCredit || null,
    featuredImageSourceUrl: d.featuredImageSourceUrl || null,
    ...
    contributorName: d.contributorName || null,
    contributorBio: d.contributorBio || null,
    sources: d.sources,
    faqs: d.faqs,
