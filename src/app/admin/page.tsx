  ...
  const [[k], searchDaily, toolDaily, subDaily, pubDaily, recent, zero, jobs, ingest, allowance, queue] = await Promise.all([
    rawQuery<Record<string, number>>(
      ...
      sql`select
        ...
        (select count(*) from articles where status = 'published' and published_at > ${Date.now() - 7 * 86_400_000}) as published_week,
        (select count(*) from articles where status = 'published' and published_at between ${Date.now() - 14 * 86_400_000} and ${Date.now() - 7 * 86_400_000}) as published_prev,
        (select count(*) from search_queries where created_at > ${Date.now() - 7 * 86_400_000}) as searches_week,
        (select count(*) from search_queries where created_at between ${Date.now() - 14 * 86_400_000} and ${Date.now() - 7 * 86_400_000}) as searches_prev,
        (select count(*) from search_queries where created_at > ${Date.now() - 7 * 86_400_000} and result_count = 0) as zero_week,
        (select count(*) from tool_runs where created_at > ${Date.now() - 7 * 86_400_000}) as runs_week,
        (select count(*) from tool_runs where created_at between ${Date.now() - 14 * 86_400_000} and ${Date.now() - 7 * 86_400_000}) as runs_prev,
        (select count(*) from newsletter_subscribers where status = 'active') as subs,
        (select count(*) from newsletter_subscribers where created_at > ${Date.now() - 7 * 86_400_000}) as subs_week,
        (select count(*) from newsletter_subscribers where created_at between ${Date.now() - 14 * 86_400_000} and ${Date.now() - 7 * 86_400_000}) as subs_prev,
        ...
        (select count(*) from businesses where status = 'active' and is_verified = 1) as verified,
        (select coalesce(sum(amount_pkr), 0) from orders where status in ('paid', 'active', 'expired') and paid_at > ${Date.now() - 30 * 86_400_000}) as revenue_30,
        (select coalesce(sum(amount_pkr), 0) from orders where status in ('paid', 'active', 'expired') and paid_at between ${Date.now() - 60 * 86_400_000} and ${Date.now() - 30 * 86_400_000}) as revenue_prev,
        (select count(*) from analytics_events where name = 'business_click' and created_at > ${Date.now() - 7 * 86_400_000}) as clicks_week,
        (select count(*) from posts where status = 'published' and published_at > ${Date.now() - 7 * 86_400_000}) as posts_week,
        (select count(*) from comments where status = 'published' and created_at > ${Date.now() - 7 * 86_400_000}) as comments_week,
        (select count(*) from professionals where status = 'active') as pros`,
    ),
    rawQuery<Daily>(db, sql`select date(created_at / 1000, 'unixepoch') as d, count(*) as n from search_queries where created_at > ${Date.now() - 14 * 86_400_000} group by 1`),
    rawQuery<Daily>(db, sql`select date(created_at / 1000, 'unixepoch') as d, count(*) as n from tool_runs where created_at > ${Date.now() - 14 * 86_400_000} group by 1`),
    rawQuery<Daily>(db, sql`select date(created_at / 1000, 'unixepoch') as d, count(*) as n from newsletter_subscribers where created_at > ${Date.now() - 14 * 86_400_000} group by 1`),
    rawQuery<Daily>(db, sql`select date(published_at / 1000, 'unixepoch') as d, count(*) as n from articles where status = 'published' and published_at > ${Date.now() - 14 * 86_400_000} group by 1`),
    db.query.articles.findMany({ orderBy: [desc(schema.articles.updatedAt)], limit: 8, with: { category: true } }),
    rawQuery<{ query: string; n: number }>(db, sql`select normalized as query, count(*) as n from search_queries where created_at > ${Date.now() - 7 * 86_400_000} and result_count = 0 group by normalized order by n desc limit 6`),
    ...
    rawQuery<Record<string, number>>(
      ...
      sql`select
        ...
        (select count(*) from articles where status = 'scheduled' and scheduled_for < ${Date.now() + 86_400_000}) as due`,
    ),
  ]);
  const q = queue[0] ?? {};
  const attention = [
    ...
    { n: q.posts, label: "community posts to approve", href: "/admin/community" },
    { n: q.community_reports, label: "reported posts, comments or members", href: "/admin/community?view=reported" },
