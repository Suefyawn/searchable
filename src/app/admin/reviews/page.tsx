    ...
    <div>
      ...
      <div className="mb-4 flex gap-2">
        {["pending", "published", "hidden", "all"].map((s) => (
          <Link key={s} href={`/admin/reviews?status=${s}`} className={cn("rounded-full border px-3 py-1 text-sm", status === s ? "border-brand-600 bg-brand-700 text-white" : "border-line bg-surface text-2")}>
            {s}
          </Link>
        ))}
      </div>
