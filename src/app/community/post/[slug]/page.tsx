...
export default async function PostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const jsonLd =
    p.kind === "job"
      ? { "@context": "https://schema.org", "@type": "JobPosting", title: p.title, description: p.body.slice(0, 2000), datePosted: p.publishedAt?.toISOString(), validThrough: m.deadline, employmentType: m.employmentType?.toUpperCase(), hiringOrganization: m.company ? { "@type": "Organization", name: m.company } : undefined, jobLocation: p.city ? { "@type": "Place", address: { "@type": "PostalAddress", streetAddress: p.city.name, addressLocality: p.city.name, addressRegion: p.city.parent?.name ?? p.city.name, addressCountry: "PK" } } : { "@type": "Place", address: { "@type": "PostalAddress", addressLocality: "Pakistan", addressRegion: "Pakistan", addressCountry: "PK" } }, jobLocationType: p.city ? undefined : "TELECOMMUTE", applicantLocationRequirements: p.city ? undefined : { "@type": "Country", name: "Pakistan" }, baseSalary: m.salaryMin || m.salaryMax ? { "@type": "MonetaryAmount", currency: "PKR", value: { "@type": "QuantitativeValue", minValue: m.salaryMin, maxValue: m.salaryMax, unitText: "MONTH" } } : undefined }
