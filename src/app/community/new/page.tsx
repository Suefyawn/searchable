...
export default async function NewPostPage({ searchParams }: { searchParams: Promise<{ kind?: string; topic?: string }> }) {
  ...
  return (
    <div className="container-x py-8 sm:py-10">
      ...
      <PostForm initial={{ kind: k ?? "discussion", topic: topic?.slice(0, 40) || undefined, images: [], negotiable: false }} cities={cities} isEditor={hasRole(user, "editor")} />
    </div>
  );
}
