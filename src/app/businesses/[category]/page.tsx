      ...
      {cities.length ? (
        <section className="mb-10">
          ...
          <ul className="flex flex-wrap gap-2">
            {cities.map((c) => (
              <li key={c.id}>
                <Link href={`/businesses/${cat.slug}/${c.slug}`} className="inline-flex items-center gap-2 rounded-full border border-line bg-surface px-3.5 py-1.5 text-sm hover:border-brand-400">
