...
export const POST = withAdminApi(async (_req, { body }) => {
  ...
  if (d.image && "query" in d.image) {
    const strict = d.kind === "news";
    const img = await findAndImport(d.image.query, "article", d.image.alt ?? d.title, { fallbackQuery: d.image.fallbackQuery, budgetMs: 25_000, entities: d.image.entities ?? entitiesIn(d.title), headline: d.title, strict });
    ...
    else imageNote = strict ? `No openly licensed photo describes this story ("${d.image.query}", entities ${JSON.stringify(d.image.entities ?? entitiesIn(d.title))}); published without one rather than with a loosely related photo (ADR-51). Pass a known photo as image.url if you have one.` : `No openly licensed photo found for "${d.image.query}" right now; the photo backfill job will try again over the next days`;
