 ...
 *   { search }                     list openly licensed candidates from Openverse (no import)
...
export const POST = withAdminApi(async (_req, { body }) => {
  ...
  if ("search" in d) return { candidates: await searchOpenImages(d.search, { limit: 12, orientation: d.orientation, minWidth: 800 }) };
  if ("query" in d) {
